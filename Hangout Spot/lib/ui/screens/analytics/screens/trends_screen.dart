...
class _TrendsScreenState extends ConsumerState<TrendsScreen> {
  ...
  Widget _buildContent(AnalyticsData data) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          // Page Title
          Text('Sales Trends', style: AnalyticsTheme.headingMedium),
          const SizedBox(height: 4),
          Text(
            'Analyzing patterns for ${dateFormat.format(_startDate)} - ${dateFormat.format(_endDate)}',
  ...
  Widget _buildStaffingAdvisor(AnalyticsData data) {
    final busiest = data.dayOfWeekOrders.reduce(
      (a, b) => a.count > b.count ? a : b,
    );
    final slowest = data.dayOfWeekOrders.reduce(
      (a, b) => a.count < b.count ? a : b,
    );
    const days = [
      'Monday',
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Busiest: ${days[busiest.dayOfWeek]} (${busiest.count} orders) — consider +1 staff.',
                  style: const TextStyle(
                    color: AnalyticsTheme.primaryText,
                    fontSize: 14,
                  ),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Slowest: ${days[slowest.dayOfWeek]} (${slowest.count} orders) — good for prep/cleaning.',
                  style: const TextStyle(
                    color: AnalyticsTheme.primaryText,
                    fontSize: 14,
                  ),
